...
private fun SeoroSeogaShApp(
    ...
    val myBooks = remember { mutableStateListOf(*loadMyBooks(context).toTypedArray()) }
    ...
    fun addMyBook(
        ...
        myBooks.add(0, book)
        saveMyBooks(context, myBooks)
        return book
    }

    fun removeMyBook(book: MyBook) {
        myBooks.removeAll { it.myBookId == book.myBookId }
        readingLogs.removeAll { it.bookId == book.myBookId }
        saveMyBooks(context, myBooks)
    }
    ...
    when (val current = screen) {
        ShScreen.Home -> HomeScreen(
            ...
            onCreateMeetingClick = {
                context.startActivity(Intent(context, MeetRegActivity::class.java))
            },
            onMeetingClick = {
                selectedMeeting = it
                context.startActivity(
                    Intent(context, ParticipateActivity::class.java)
                        .putExtra(ParticipateActivity.EXTRA_MEETING_ID, it.meetingId)
                )
            },
        ...
        ShScreen.MyPage -> MyPageScreen(
            ...
            onDeleteBookClick = ::removeMyBook
        )
...
private const val MyBooksPrefsName = "my_books_local"
private const val MyBooksPrefsKey = "books"

private fun loadMyBooks(context: Context): List<MyBook> {
    val prefs = context.getSharedPreferences(MyBooksPrefsName, Context.MODE_PRIVATE)
    if (!prefs.contains(MyBooksPrefsKey)) return sampleMyBooks()
    return loadSavedMyBooks(context)
}

private fun loadSavedMyBooks(context: Context): List<MyBook> {
    val raw = context.getSharedPreferences(MyBooksPrefsName, Context.MODE_PRIVATE)
        .getString(MyBooksPrefsKey, null)
        ?: return emptyList()
    return runCatching {
        ...
        buildList {
            for (index in 0 until array.length()) {
                ...
                add(
                    MyBook(
                        myBookId = item.optString("myBookId").ifBlank { UUID.randomUUID().toString() },
                        title = item.optString("title"),
                        author = item.optString("author"),
                        publisher = item.optString("publisher"),
                        coverImageUri = item.optString("coverImageUri").takeIf { it.isNotBlank() },
                        coverImageRes = item.optInt("coverImageRes", 0).takeIf { it != 0 },
                        bookImageUrl = item.optString("bookImageUrl").takeIf { it.isNotBlank() },
                        description = item.optString("description").takeIf { it.isNotBlank() },
                        isbn = item.optString("isbn").takeIf { it.isNotBlank() },
                        totalPage = item.optInt("totalPage", 300).coerceAtLeast(1),
...
private fun saveMyBooks(context: Context, books: List<MyBook>) {
    val array = JSONArray()
    books.forEach { book ->
        array.put(
                JSONObject()
                    .put("myBookId", book.myBookId)
                    .put("title", book.title)
                    .put("author", book.author)
                    .put("publisher", book.publisher)
                    .put("coverImageUri", book.coverImageUri.orEmpty())
                    .put("coverImageRes", book.coverImageRes ?: 0)
                    .put("bookImageUrl", book.bookImageUrl.orEmpty())
                    .put("description", book.description.orEmpty())
                    .put("isbn", book.isbn.orEmpty())
                    .put("totalPage", book.totalPage)
                    .put("addedAtMillis", book.addedAtMillis)
            )
    }

    context.getSharedPreferences(MyBooksPrefsName, Context.MODE_PRIVATE)
        .edit()
        .putString(MyBooksPrefsKey, array.toString())
        .apply()
}
