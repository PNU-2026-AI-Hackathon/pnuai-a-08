...
> 아래는 **현재 프로젝트 구현 컨텍스트를 기준으로 정리한 최종 스택**입니다.  
### Frontend
...
<p align="center"><sub>책 발견과 캠퍼스 대여부터 AI 독서 지원, 기록, 콘텐츠·커뮤니티를 거쳐 다음 책을 다시 발견하는 순환형 사용자 여정</sub></p>
...
공개 README에는 개인 연락처 대신 **GitHub Repository Issues**를 공식 문의 채널로 사용합니다.

---

# 7. 해커톤 참여 후기

> 아래 내용은 팀원별 역할과 프로젝트 진행 과정에서 얻은 핵심 학습을 중심으로 정리했습니다.

### 김민주

기획과 개발을 분리해서 진행하기보다, 기능 우선순위와 백엔드 구조를 함께 맞추는 것이 MVP 완성도에 직접적인 영향을 준다는 것을 느낄 수 있었습니다. 특히 짧은 개발 기간에는 모든 기능을 구현하는 것보다 핵심 사용자 흐름이 끊기지 않도록 범위를 조절하는 방법을 배울 수 있었습니다.
### 정승한

AI 기능을 서비스에 넣는 것 자체보다, 어떤 시점에 어떤 맥락을 제공해야 실제 사용자의 독서를 돕는지가 더 중요했습니다. AI와 백엔드 기능을 독립 모듈로 나누고 기능 간 계약을 명확히 하는 과정이 유지보수와 협업에 도움이 되었습니다.
...
시장조사와 경쟁 서비스 분석을 통해 단순히 기능을 많이 추가하는 것보다, 실제 대학생이 겪는 도서 접근성과 독서 지속 문제를 명확히 정의하는 것이 중요했습니다. 비개발 관점의 문제 정의가 개발 우선순위를 정하는 데에도 직접적으로 연결될 수 있음을 확인했습니다.
